                                ...
                                @foreach ($notas as $nota)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                        ...
                                        <td class="px-4 py-2 text-center space-x-2">
                                            <form action="{{ route('notas.aprovar', $nota) }}" method="POST" class="inline-block">
                                                @csrf
                                                <button type="submit" class="text-green-600 hover:underline">Aprovar</button>
                                            </form>
                                            <form action="{{ route('notas.reprovar', $nota) }}" method="POST" class="inline-block" onsubmit="return confirm('Tem certeza que deseja reprovar esta nota?')">
                                                @csrf
